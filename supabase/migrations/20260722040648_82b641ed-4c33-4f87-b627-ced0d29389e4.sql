...
CREATE OR REPLACE FUNCTION public.get_public_order(_token uuid)
 RETURNS TABLE(id uuid, status text, order_type text, total numeric, notes text, created_at timestamp with time zone, delivery_eta timestamp with time zone, delivery_address jsonb, restaurant_address text, items jsonb)
 LANGUAGE sql
