...
ALTER TABLE public.settings
  ADD COLUMN IF NOT EXISTS avg_prep_minutes integer NOT NULL DEFAULT 25,
...
CREATE OR REPLACE FUNCTION public.get_public_order(_token uuid)
 RETURNS TABLE(id uuid, status text, order_type text, total numeric, notes text, created_at timestamptz, delivery_eta timestamptz, delivery_address jsonb, items jsonb)
