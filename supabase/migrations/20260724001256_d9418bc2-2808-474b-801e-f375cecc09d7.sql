...
CREATE OR REPLACE FUNCTION public.get_order_payment_status(_token uuid)
 RETURNS TABLE(id uuid, payment_status text, mp_qr_code text, mp_qr_code_base64 text, mp_ticket_url text, status text, payment_method text, mp_public_key text, total numeric)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  SELECT o.id, o.payment_status, o.mp_qr_code, o.mp_qr_code_base64, o.mp_ticket_url, o.status::text,
         o.payment_method, r.mp_public_key, o.total
...
BEGIN
  ...
  v_payment_status := CASE
    WHEN _payment_method IN ('pix', 'credit_card_online') THEN 'pending'
    ELSE NULL
  END;
